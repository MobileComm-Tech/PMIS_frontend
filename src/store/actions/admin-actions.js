...
const AdminActions = {
  ...
  getPartnerWorkDescription:
    ...
    async (dispatch, _) => {
      try {
        const res = await Api.get({
          url: `${Urls.admin_partner_work_description}${
            args != "" ? "?" + args : ""
          }`,
          show: show,
        });
  ...
  patchComplinaceSnapImageSubmition:
    (data, cb, siteId = "", milestoneId = "") =>
    async (dispatch, _) => {
      try {
        const res = await Api.patch({
          data: data,
          url:
            ...
            (siteId && milestoneId ? `/${siteId}/${milestoneId}` : "") +
            "?forFile=true",
          contentType: "multipart/form-data",
        });
        if (res?.status !== 201 && res?.status !== 200) {
  ...
  getManageCostCenter:
    ...
    async (dispatch, _) => {
      try {
        let urlName = `${Urls.admin_cost_center}${
          args != "" ? "?" + args : ""
        }`;
        if (id) {
          urlName = `${Urls.admin_cost_center}/${id}${
            args != "" ? "?" + args : ""
          }`;
        }
  ...
  getManageDepartment:
    ...
    async (dispatch, _) => {
      try {
        ...
        if (id) {
          urlName = `${Urls.admin_department}/${id}${
            args != "" ? "?" + args : ""
          }`;
        }
        const res = await Api.get({
  ...
  getManageDesignation:
    ...
    async (dispatch, _) => {
      try {
        let urlName = `${Urls.admin_designation}${
          args != "" ? "?" + args : ""
        }`;
        if (id) {
          urlName = `${Urls.admin_designation}/${id}${
            args != "" ? "?" + args : ""
          }`;
        }
  ...
  getAccuralRevenueMasterProjectID:
    ...
    async (dispatch, _) => {
      try {
        const res = await Api.get({
          url: `${Urls.get_accural_revenue_master_project_projectId}/${id}${
            args != "" ? "?" + args : ""
          }`,
          reset,
        });
  ...
  getAccuralRevenueMasterProjectType:
    ...
    async (dispatch, _) => {
      try {
        const res = await Api.get({
          url: `${Urls.get_accural_revenue_master_project_projectType}/${id}${
            args != "" ? "?" + args : ""
          }`,
          reset,
        });
...
getEmpNameList:
    ...
    try {
      ...
      if (value < 2) {
        dispatch(
          GET_COMPLIANCE_DEGROW_TEMPLATE_DATA_USED_FIELDS({
            tabName,
            dataAll: [formFields[0], ...actualFields],
          })
        );
        return;
      }
  ...
  getSubProjectDeliveryPVA :
    ...
    async (dispatch, _) => {
      try {
        const res = await Api.get({
          url: `${Urls.admin_sub_project_delivery_PVA}/${id}${
            args != "" ? "?" + args : ""
          }`,
          reset,
        });

...
const AdvancedTableExpandableOneRow = ({
  ...
  function expendedRows() {
    ...
    const keys =
      poData?.length > 0
        ? Object.keys(poData[0]).reduce((acc, key) => {
            acc[key] = key;
            return acc;
          }, {})
        : {};
    ...
    Object.keys(finalData).map((key, index) => {
      ...
      finalData[key].forEach((item, index) => {
        ...
        table?.childs?.milestoneArray.map((innerItem) => {
          console.log(
            finalData[item?.workDescription][index]?.[
              finalData[item?.workDescription]?.[index]?.systemId +
                finalData[item?.workDescription]?.[index]?.itemCode
            ],
            finalData[item?.workDescription]?.[index]?.systemId +
              "_" +
              finalData[item?.workDescription]?.[index]?.itemCode,
                ...
                >
                  {innerItem.name === "Vendor Item Code" ? (
                    ...
                    <span className="px-4 py-[2px] bg-[#1cb99c] rounded-md">
                      ...
                      item?.itemCode ===
                        keys[item?.systemId + "_" + item?.itemCode]?.split(
                          "_"
                        )[1] &&
                      item?.systemId ===
                        keys[item?.systemId + "_" + item?.itemCode]?.split(
                        ...
                        ? finalData[0]?.[item?.systemId + "_" + item?.itemCode]
                        : item?.itemCode === undefined &&
                          item?.systemId ===
                            keys[item?.systemId + "_poStatus"]?.split("_")[0]
                        ? finalData[0]?.[item?.systemId + "_poStatus"]
                        : finalData[item?.workDescription]?.[0]?.[
                            ""
                          ]} */}
                      {finalData[item?.workDescription]?.[index]?.itemCode !==

...
const CustomInputDropdown = ({
  ...
  // console.log(quantityValue, "dvdsdscsdcd");
  ...
  useEffect(() => {
    if (
      itm?.name &&
      (quantityValue[itm.name] === undefined ||
        quantityValue[itm.name] === null ||
        quantityValue[itm.name] === "")
    ) {
  ...
  }, [itm?.name, quantityValue, setQuantityValue]);

  const currentValue = quantityValue[itm?.name] || 1;
        ...
        >
          <input
            ...
            onChange={(e) => {
              setQuantityValue((prev) => ({
                ...prev,
                [itm?.name]: Number(e?.target?.value) || 1,
              }));
            }}
          />
          <div className="absolute inset-y-0 right-2 flex items-center text-gray-700 text-sm cursor-pointer">
            ▼
          </div>

...
const VendorCost2 = () => {
  ...
  let dbConfigList = useSelector((state) => {
    let interdata = state?.vendorData?.getVendorCostMilestoneSecond || [];
    return interdata?.map((itm) => {
      let updateditm = {
        ...itm,
        edit: (
  ...
  // console.log(projectTypeList,"dsfsdfsffd")
  ...
  console.log(subProjectTypeList,"ttttttttt")
  ...
  const onTableViewSubmit = (data) => {
    data["fileType"] = "vendorCostSecond";
    // data['collection'] = "vendorCost"
    dispatch(
      CommonActions.fileSubmit(Urls.common_file_uploadr, data, () => {
  ...
  const onSubmit = (data) => {
    ...
    setstrVal(strVal);
    dispatch(
      VendorActions.getVendorCostMilestoneSecond(true, objectToQueryString(data))
    );
    // dispatch(
    //   AdminActions.getAccuralRevenueMasterProject(
    //     true,
  ...
  return (
    <>
      ...
      <FileUploader
        ...
        setIsOpen={setFileOpen}
        tempbtn={true}
        tempbtnlink={["/template/VendorCost.xlsx", "Vendor Cost.xlsx"]}
      />
    </>
  );
};
export default VendorCost2;

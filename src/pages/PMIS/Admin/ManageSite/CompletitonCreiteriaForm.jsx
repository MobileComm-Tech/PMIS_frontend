...
// import { calculateCompletionCriteriaPaylaod, quantitySelectTypeOptions, range } from "../../../../components/CommonObjectsAndVariables";
...
// const quantityOptions =quantitySelectTypeOptions()
//   useEffect(()=>{
    
//      const itemCodeData = responseData?.data?.data!==undefined? responseData?.data?.data[0]:[];
...
//               console.log(e?.target?.value,"___vajhvbds")
//               setQuantityValue(prev => ({
//                                   ...prev,
//                                   [key]: e?.target?.value==="customQuantity"?"customQuantity":Number(e?.target?.value)
//                                 }));
//             },
//           },
...
// //   const qtyKey = `quantity0${i}`;
// //   const itemCode = `itemCode0${i}`;
      
// //   if(quantityValue[qtyKey]==="customQuantity"){
     
// //       console.log(itemCodeAllInputs,"___itemCodeAllInputs__")
// //       if(itemCodeAllInputs[i*2]?.name===qtyKey){
// //         setItemCodeAllInputs(prev => prev.map((item, index) => index === i*2 ? { ...item, type: "number" } : item));
  
// //     setValue(itemCode,getValues(itemCode));
// //     console.log(getValues(itemCode),"__getValue")
// //     // console.log(setValue(itemCode,getValues(itemCode)),"___getttingValue")
// //     // getValues(rateKey)
// //       }
// //        quantityValue[qtyKey]=1;
// //   }
// //   else if (quantityValue[rateKey] !== undefined || NaN && quantityValue[qtyKey] !== undefined || NaN ) {
// //     // console.log(quantityValue[rateKey],quantityValue[qtyKey],"___fghjk")
// //     total += Number(quantityValue[rateKey]) * Number(quantityValue[qtyKey]);
// //   }
// // }
// // ============================new
// for (let i = range.start; i <= range?.end; i++) {
//   const paddedIndex = String(i).padStart(2, '0');
//   const rateKey = `itemCodeRate${paddedIndex}`;
//   const qtyKey = `quantity${paddedIndex}`;
//   const itemCode = `itemCode${paddedIndex}`;

//   if (quantityValue[qtyKey] === "customQuantity") {
    
//     const currentValue = getValues(itemCode);

//     if (itemCodeAllInputs[i * 2]?.name === qtyKey && itemCodeAllInputs[i * 2]?.type !== "number") {
     
//       setItemCodeAllInputs(prev =>
//         prev.map((item, index) =>
//           index === i * 2
//             ? { ...item, type: "number" }
//             : item
...
//       // setTimeout(() => {
//       //   setValue(itemCode, currentValue);
//       // }, 0);
//       setTimeout(() => {
...
//     quantityValue[qtyKey] = 1;
//   } else {
//     const rate = Number(quantityValue?.[rateKey] ?? 0);
//     const qty = Number(quantityValue?.[qtyKey] ?? 0);
//     total += rate * qty;
//   }
// }
...
//   },[quantityValue,itemCodeAllInputs])

//   console.log(quantityValue, "quantityValue")



// console.log(itemCodeAllInputs,"__wertyuio")

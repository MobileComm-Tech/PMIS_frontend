...
import {
  calculateCompletionCriteriaPaylaod,
  range,
...
const CompletitonCreiteriaForm = ({
  ...
  let lastMonthYear = currYear;

  if (lastMonth === 0) {
    lastMonth = 12;
    lastMonthYear = currYear - 1;
  }

  const lastMonth26 = new Date(lastMonthYear, lastMonth - 1, 26);

  if (datestr < lastMonth26) {
    datestr = lastMonth26;
  }
    ...
    for (let i = range.start; i <= range.end; i++) {
      ...
      if (itemCodeData[key]?.length > 0) {
        const filteredData = itemCodeData[key]?.filter((item) => {
          if ((item?.rate || item?.rate === 0) && item?.itemCodeDescription)
            return item;
        });
        const tempData = {
          label: `Item Code- 0${i}`,
          ...
          option: filteredData?.map((itm, index) => {
            if (itm?.rate !== undefined || itm?.rate !== '') {
              return {
                ...
                value:
                  itm?.value + ',' + itm?.rate + ',' + itm?.itemCodeDescription,
                rate: itm?.rate,
              };
            }
          }),
  ...
  useEffect(() => {
    ...
    for (let i = range.start; i <= range?.end; i++) {
      ...
      if (
        quantityValue[rateKey] !== undefined &&
        !isNaN(quantityValue[rateKey]) &&
        quantityValue[qtyKey] !== undefined &&
        !isNaN(quantityValue[qtyKey])
      ) {
        total += quantityValue[rateKey] * quantityValue[qtyKey];
        console.log(
          quantityValue[rateKey],
          quantityValue[qtyKey],
          total,
          '___total__',
        );
      }
    ...
    return mileStone['Completion Criteria']?.split(',').map((dta) => {
      ...
      if (dta == 'Forms & Checklist') {
        return {
          ...
          component: (
            <p
              ...
              onClick={() => {
                dispatch(
                  GET_GLOBAL_COMPLAINCE_TYPE_DATA({ dataAll: [], reset: true }),
                );
                if (projectTypeName !== 'DEGROW') {
                  dispatch(
                    GET_ONE_COMPLIANCE_L1_LIST({ dataAll: [], reset: true }),
                  );
                  dispatch(
                  ...
                  dispatch(
                    AdminActions.getOneComplianceL1List(
                      siteCompleteData.uniqueId,
                      mileStone.Name,
                      true,
                      '',
                ...
                setmodalFullBody(
                  projectTypeName === 'DEGROW' && milestoneName === 'Survey' ? (
                    ...
                    (milestoneName === 'SRQ Raise' ||
                      milestoneName === 'Dismantle') ? (
        ...
        return {
          ...
          option:
            geeter?.[0]?.type == 'Dropdown'
              ? geeter[0]['dropdown']?.split(',').map((itm) => {
                  return {
                    label: itm,
                    value: itm,
                  };
  ...
  let backgeturl = projectListActions.getProjectTypeAll(
    projectuniqueId,
    filterView,
  );
  if (myTaskPage === 'Yes') {
  ...
  const onsubmiting = (data) => {
    if (checkmilestone.includes('Forms & Checklist')) {
      ...
      for (let i = range.start; i <= range.end; i++) {
        ...
        if (['MS1']?.includes(mileStone?.Name)) {
          if (quantityValue[quantityKey] !== undefined) {
            tempData = {
              ...tempData,
              [quantityKey]: quantityValue[quantityKey],
            };
          }
          // else {
      ...
      data = { ...data, ...tempData };
      data = calculateCompletionCriteriaPaylaod(
        itemCodeAllInputs,
        data,
        mileStone?.Name,
      );
  ...
  return (
    <>
      ...
      <div className="flex justify-center">
        ...
        {checkmilestone?.includes('Forms & Checklist') &&
          ['Open', 'In Process', 'Reject']?.includes(checkmilestoneStatus) && (
            <Button
              onClick={handleSubmit(onsubmiting)}
              name={'Submit'}
              classes="w-auto"

...
const VendorGroupTaskAllocation = ({
  ...
  console.log(setFilteredVendorNames, "setFilteredVendorNames");
  ...
  // console.log(selectedWorkDescription, "selectedWorkDescription");
    ...
    console.log("selectedMilestoneData received:", selectedMilestoneData);
    console.log("itemData:", itemData);
    
    if (formName === "Deallocate Task") {
      console.log("=== DEALLOCATE TASK MODE ===");
      
      // Check milestone data structure
      selectedMilestoneData.forEach((milestone, index) => {
        console.log(`Milestone ${index + 1}:`, {
          milestoneId: milestone.milestoneId,
          milestoneName: milestone.milestoneName,
          assignerResult: milestone.assignerResult,
          siteData: milestone.siteData
        });
        
        if (milestone.assignerResult && milestone.assignerResult.length > 0) {
          console.log(`Milestone ${index + 1} - Assigners:`, 
            milestone.assignerResult.map(a => ({
              assignerName: a.assignerName,
              assignerId: a.assignerId || a.id
            }))
          );
        } else {
          console.log(`Milestone ${index + 1} - No assigners found`);
        }
      });
    }
  }, [selectedMilestoneData, formName]);
        ...
        projectListActions.VendorList(
          ...
          (response) => {
            ...
            if (
              ...
              // console.log(vendorOptions,"vendorOptions___")
  ...
  let Form = [
    ...
    },
    {
      ...
      name: "groupMilestone",
      value:
        watchGroupMilestone?.length > 0
          ? watchGroupMilestone?.split(":;")[1]
          : "",
      required: true,
      type: "sdisabled",
      classes: "col-span-1",
  ...
  if (formName == "Deallocate Task") {
  const vendorName =
    dataGetterOld?.vendorDetails?.[0]?.name || "No Vendor Available";

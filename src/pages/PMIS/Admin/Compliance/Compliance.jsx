...
//                               `${Urls.post_Wcc_Compliance}/${itm?.uniqueId}`,
//                               () => {
//                                 dispatch(AdminActions.getWccCompiliance());
//                               },
//                               itm?.uniqueId,
...
//               return { label: itm?.projectType, value: itm?.projectType };
//             })
//           : [],
//         props: {
//           onChange: (e) => {
...
//                   true,
//                   `customerId=${selectedCustomerOption}&projectType=${e.target.value}`,
//                 ),
//               );
//             } else {
//               dispatch(
...
const Compliance = () => {
  const [modalOpen, setmodalOpen] = useState(false);
  const [modalHead, setmodalHead] = useState("");
  const [modalFormValue, setModalFormValue] = useState({}); // ← store data, not JSX
          ...
          <CstmButton
            ...
            child={
              <EditButton
                name={""}
                onClick={() => {
                  console.log(itm, "__itm");
                  setModalFormValue(itm); // store the row data
                  setmodalHead("Edit Compliance");
                  setModalKey((k) => k + 1); // bump forces fresh remount
                  setmodalOpen(true);
                }}
              ></EditButton>
            }
  ...
  return (
    <>
        ...
        headerButton={
          <div className="flex">
            <ConditionalButton
              ...
              // onClick={() => navigate("/empdetails")}
              onClick={() => {
                setModalFormValue({}); // empty = add mode
                setmodalHead("Add Compliance");
                setModalKey((k) => k + 1);
                setmodalOpen(true);
              }}
      ...
      <Modal
        ...
        children={
          <ComplianceForm
            key={modalKey} // ← THIS is what actually forces full remount

...
const WorkDone = () => {
  ...
  let table = {
    columns: [
      ...
      },
      {
      ...
      },
       {
        name: "Unique ID",
        value: "uniqueID",
        style: "min-w-[160px] max-w-[250px] text-center bg-[#3e454d] z-10",
      },
      {
        name: "Site ID",
        value: "Site Id",
    ...
    filter: [
   {
  label: "MS1 Completion",
  type: "datetimeRangeNew",
  name: "ms1Completion",
render: ({ value, onChange }) => {
  console.log("MS1 value from AdvancedTable:", value);
...
},
{
  ...
  const onSubmit = (data) => {
    let shouldReset = data.reseter;
    delete data.reseter;
     if (data.ms1Completion?.start && data.ms1Completion?.end) {
  data.ms1StartDate = moment(data.ms1Completion.start).format("YYYY-MM-DD");
  data.ms1EndDate = moment(data.ms1Completion.end).format("YYYY-MM-DD");
}
delete data.ms1Completion;

if (data.ms2Completion?.start && data.ms2Completion?.end) {
  data.ms2StartDate = moment(data.ms2Completion.start).format("YYYY-MM-DD");
  data.ms2EndDate = moment(data.ms2Completion.end).format("YYYY-MM-DD");
}
delete data.ms2Completion;
    let strVal = objectToQueryString(data);
    setstrVal(strVal);
    dispatch(FinanceActions.getPOWorkDoneBased(true, "", strVal,customerId));
  };

...
// import VendorGroupTaskAllocation from "../Admin/ManageProjectSiteId/VendorGroupTaskAllocation";
// // import ManageVendorPartnerTeamTaskAllocation from "../Vednor Partner Team/ManageVendorPartnerTeamTaskAllocation";
// import VendorActions from "../../../store/actions/vendor-actions";
// import ManageVendorPartnerTeamTaskAllocation from "../Vendor Partner Team/ManageVendorPartnerTeamTaskAllocation";
...
//                   setItemData(
//                     itm?.milestoneArray?.map((itm) => itm?.uniqueId)?.join(",")
//                   );
//                 } else {
//                   setparentsite((prev) => {
...
//                       onClick={() => {
//                         if (
//                           ["L1-Rejected", "L2-Rejected"].includes(
//                             iewq?.ptwStatus
//                           )
//                         ) {
...
//                           setmodalBody(
//                             <RejectionReason
//                               itemData={itm?.milestoneArray[index]}
//                             />
//                           );
//                           setmodalOpen(true);
...
//                       });
//                     }
//                     console.log(iewq?.uniqueId, "___iewq?.uniqueId__");
//                     if (!itemData?.includes(iewq?.uniqueId)) {
//                       setItemData((prev) =>
//                         prev?.length === 0
//                           ? prev + iewq?.uniqueId
//                           : prev + "," + iewq?.uniqueId
...
//                     } else if (
//                       itemData?.length > 0 &&
//                       itemData?.includes(iewq?.uniqueId)
//                     ) {
//                       const newIds = itemData
//                         ?.split(",")
//                         ?.filter((itm) => itm !== iewq?.uniqueId)
//                         ?.join(",");
//                       console.log(newIds, "___newIds__");
//                       setItemData(newIds);
//                     }
//                   }}
//                 />
...
//   console.log(itemData, "___utenadsd");
...
//       setPtwDriveTest(true);
//       setVehicleType(
//         allFormType.includes("roadsafetychecklist4Wheeler")
...
//     if (isPhoto) {
//       if (
//         allFormType?.includes("ptwphoto4wheeler") ||
//         allFormType?.includes("ptwphoto2wheeler")
//       ) {
//         return true;
//       } else {
...
//       if (
//         allFormType?.includes("roadsafetychecklist4Wheeler") ||
//         allFormType?.includes("roadsafetychecklist2Wheeler")
//       ) {
//         return true;
//       } else {
//         return false;
...
//             {itemData?.length > 0 && (
//               <ConditionalButton
//                 showType={getAccessType("Partner Team Allocation")}
//                 classes="mr-1 bg-[#ebad5d]"
//                 onClick={(e) => {
//                   const uniqueArr = [...new Set(subProjectId)];
//                   console.log(itemData, "___");
...
//                     //   )
//                     // );
//                     setmodalHead("Partner Team Allocation");
//                     setmodalBody(
//                       <ManageVendorPartnerTeamTaskAllocation
//                         from={"bulktask"}
//                         listsite={parentsite}
//                         itemData={itemData}
...
//                     [
//                       ...allFormType,
//                       ...(isForDriveTest() ? ["roadSafetyChecklist"] : []),
//                       ...(isForDriveTest(true) ? ["ptwphoto"] : []),
//                     ].includes(item.id)
//                   )
//                   .map((option) => (
//                     <label
...
//           driveFormModel && (
//             <CommonFormPTW
//               isPtwRaise={isPtwRaise}
//               setDriveFormModel={setDriveFormModel}
//               getApprovalsData={getApprovalsData}
//               setPtwModalHead={setPtwModalHead}
//               formName="drivetestactivity"
//               fillData={isRaiseFormData.current}
//               formData={mileStoneItemRef.current}
//             />
//           )
//         }
...
const MyTask = () => {
  const DECLARATION_MSG = `By submitting this PTW, I confirm that all the details filled in this form are completely true and genuine. I also confirm that I and my team will strictly follow Mobilecomm Zero Tolerance Safety policy during the execution of the activity. I take full responsibility for the safety and compliance of my team members at the site. In case any member of my team breaches the safety compliance, I will immediately inform the Circle EHS team on an immediate basis.`;
  let permission = JSON.parse(localStorage.getItem("permission")) || {};
  let user = JSON.parse(localStorage.getItem("user"));
  let rolename = user?.roleName;
  const { projectuniqueId } = useParams();
  ...
  const handleApprovalConfirm = () => {
    dispatch(
      ALERTS({
        show: true,
        icon: "warning",
        text: DECLARATION_MSG,
        showCancelButton: true,
        confirmButtonText: "Accept",
        onConfirm: () => handleApprovalData(DECLARATION_MSG),
      }),
    );
  };
  const getApprovalsData = async (operationId = "") => {
        ...
        <>
          <div className="w-full flex flex-col items-scenter min-h-[50vh] max-h-full ">
            <div className="max-w-md mx-auto p-2  flex flex-col gap-3 rounded-lg shadow-md">
              ...
              <button
                onClick={(e) => handleApprovalConfirm()}
                className="w-full bg-[#13B497] text-white py-2 px-4 rounded-lg hover:bg-[#0c8b74] transition-colors duration-200 font-medium"
              >
                Submit
              </button>
  ...
  const handleApprovalData = async () => {
    ...
    if (res?.status === 200) {
      ...
      //   url: `/ptw/declaration/${res?.data[0]?.ptwNumber}`,
      //   data: {
      //     declaration: res?.data?.msg,
      //   },
      // });
                    ...
                    <span
                      onClick={() => {
                        if (
                          ...
                          setSelect(false);
                          setSelectedItems([]);
                          if (!iewq?.isPtwTaskAllowed) return;

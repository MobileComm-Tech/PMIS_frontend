...
const CommonFormPTW = ({ formName, getApprovalsData, isPtwRaise, fillData = null, setDriveFormModel, setPtwModalHead, formData }) => {
  const {
    register,
    handleSubmit,
    SubmitTask,
    ...
    if (isPtwRaise) {
      subFormRef.current[which]?.forEach((item) => {
        if (['checklist', 'photo'].includes(which)) {
          if (fillData !== null && Object.keys(fillData).includes[which]) {
  ...
  const handleContinue = (opId) => {
    ...
    if (firstForm.id === 'roadSafetyChecklist') {
      ...
      if (allFormType.includes('roadsafetychecklist2wheeler')) {
        if (isPtwRaise) {
          if (fillData !== null && Object.keys(fillData).includes['roadsafetychecklist2wheeler']) {
            setValueForm2(item?.fieldName, fillData['roadsafetychecklist2wheeler'][item?.fieldName])
          } else {
            setValueForm2(item?.fieldName, formData[item?.fieldName])
          }
  ...
  const handleVehiclePhoto = async (formDataInput, subForm) => {
    ...
    // Shared metadata
    formDataV.append("projectID", formData?.projectId);
    formDataV.append("projectuniqueId", formData?.projectuniqueId);
    formDataV.append("siteUid", formData?.siteUid);
    formDataV.append("siteId", formData?.siteId);
    formDataV.append("customerName", formData?.customerName);
    formDataV.append("circle", formData?.CIRCLE);
    ...
    formKeys?.filter((key) => {
      if (Object.keys(formDataInput).includes(key)) {
        ...
        if (value instanceof FileList) {
          formDataV.append(finalKey, value[0]);
        } else {
          formDataV.append(finalKey, value);
        }
      }
    })
